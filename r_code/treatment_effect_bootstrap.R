...
compute_tot_se_jackknife <- function(estimated_series_df, time_var = "period", treat_period_var = "Treatment_Period",
  ...
  if(post_treat_only){
    effect_series_postonly=effect_series_postonly %>%
  ...
  post_range=c(min(effect_series_postonly$post_period_t),
               max(effect_series_postonly$post_period_t))
  jk_comp_ci <-format_jackknife(
    jk_est=jk_comp_abs,ci_in=alpha_ci,col_name= 
      paste(stat_in,"_abs_tot", sep=""), range=post_range) %>%
    dplyr::inner_join(
      format_jackknife(jk_est=jk_comp_pct,ci_in=alpha_ci, col_name= 
                         paste(stat_in,"_pct_tot",sep=""), range=post_range),
      by="post_period_t")
...
compute_ci_bounds_bootstrap <- function(estimated_series_df, time_var = "period", treat_period_var = "Treatment_Period",
                                        ...
                                        stat_in = "mean", alpha_ci = 0.95,
                                        compute_cf_eff = T, counterfac_var = "counter_factual",
                                        nboots = 10000, post_treat_only=T) {
...
format_jackknife<-function(jk_est, ci_in, col_name, range){
  ...
  jk_est_ci<- 
    ...
    do.call(rbind, .) %>%
    tibble::as_tibble() %>%
    dplyr::mutate("post_period_t"=seq(range[1],range[2], 1),
