...
for_gt=list()
dataset_names=setdiff(list.files(here::here("Data", "Variations")), "Old")[1:2]
for(data_i in seq_len(length(dataset_names))){
  ...
  for_gt[[data_i]]=metrics_tib=coverage_tib %>%
    ...
    pivot_wider(names_from = c( method ), values_from=metric_value)
...
make_tables<-function(tib_to_gt_data, datalist){
  ...
  #print(all_dgp_params[[match(data_name,names(all_dgp_params))]])
    ...
    indiv_cf_plots_gsynth=plot_individual_cf(gsynth_est[[random_data]])[c(1,2)]
    indiv_cf_plots_scdid=plot_individual_cf(scdid_est[[random_data]])[c(1,2)]
    indiv_cf_plots_mc=plot_individual_cf(mc_est[[random_data]])[c(1,2)]
    indiv_cf_plots_causalimpact=
      plot_individual_cf(causalimpact_est[[random_data]])[c(1,2)]
    indiv_cf_plots_ensemble=
    ...
    do.call("grid.arrange", c(indiv_cf_combined, nrow=5, ncol=2))
    
    tsfeature_pc_by_treatment_plot(formatted_data[[random_data]])+
      ggplot2::labs(caption = data_name)
